 ...
 <section id="decisoes" class="decisoes">
    <div class="container">
        <div class="section-title">
            <h2>Deliberações da CNPD</h2> 
        </div> 
 
            <div class="col-md-12">
                <div class="form-group">
               <p class="pleft">Deliberações disponíveis: {{countNumbers()}}</p> <input id="idsearch" type="text" class="form-control" v-model="search" placeholder="Procurar por entidades ...">
                </div>
                <br>
                ...
                <table class="table table-striped table-bordered" style="width:100%">
                    ...
                    <tbody>
                        <tr v-for="(user, index) in (sortedActivity, filteredList)" :key="user.id">
                            <td>{{index + 1}}</td>
                            <td>
                            <router-link :to="{name: 'deliberaview',params: { id: user.id}}">  
                              {{user.name}} 
                            </router-link>  
                            </td> 
